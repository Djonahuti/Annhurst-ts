...
export async function GET() {
  try {
    ...
    const pageWithUrls = {
      ...page,
      id: page.id.toString(),
      team_img: page.team_img ? `/uploads/${page.team_img.split('/').pop()}` : null,
      team_img2: page.team_img2 ? `/uploads/${page.team_img2.split('/').pop()}` : null,
      team_img3: page.team_img3 ? `/uploads/${page.team_img3.split('/').pop()}` : null,
    };
